...
@interface MDesignDetailController ()<UIWebViewDelegate,UINavigationControllerDelegate>
@property(nonatomic,weak) UIProgressView *loadingProgressView;
@property(nonatomic,strong) NSTimer *progressTimer;

@end

@implementation MDesignDetailController
- (void)viewDidLoad {
    [super viewDidLoad];
    //设置发起webview以及代理
    ...
    [selectWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[self.selectedDesignF.design.design_link stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]]];
    selectWebView.delegate=self;
    //标题
    self.title=self.selectedDesignF.design.design_title;
